/**
 * RenderPipelineアーキテクチャの共通型。
 *
 * 【設計方針】
 * RenderEngine（canvasSceneRenderer）はSceneを直接描画しない。
 * 各責務（背景/カメラ/人物/エフェクト/字幕/…）を独立したProvider/後処理ステージ
 * （RenderUnit、定義は services/providers/types.ts）に分離し、RenderPipelineは
 * それらをステージ順に呼び出すだけの合成役に徹する。新しい演出は既存Providerを
 * 変更せず、新しいProviderの追加またはEffectProviderへの登録だけで拡張できる
 * （100種類以上の演出を見込んだ設計）。
 *
